...
var KTDatatableRemoteAjaxDemo=
{
	init:function()
	{
				...
				columns: [
					{
						...
						template: function(row, index) {
							...
							var tipo = {
								...
								2: {'title': 'C.E : '},
								3: {'title': 'PASAPORTE : '},
								4: {'title': 'SIN DOC DE INDENTIDAD : '},
							};

							return  tipo[row.tipo_doc].title + row.num_doc;
					...
					
					{
						...
						autoHide: false,
						template: function(row,index) {
							return '<a href="detallePersonalMedico?idprofesional='+row.idprofesional+'&nombre='+row.nombres+'" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Detalle">\
							<i class="fas fa-eye"></i>\
							</a>'+
							'<div  onclick="remover('+row.idprofesional+')" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Detalle">\
		...
		$('#kt_form_status').on('change', function() {
			datatable.setDataSourceParam("tipo_planilla",$(this).val());
...
//remover
function remover(idprofesional){
...
function remover(idprofesional){
	...
	$.ajax({
		'url': "validacion_eliminar_horario",
		'data':{'idprofesional':$("#idprofesional").val()},
		
	}).done( function(data) {
		obj = JSON.parse(data);
		var content=obj.message;
		$('#exampleModalLongTitle').html(content);
