...
class CondicionesController extends Controller
{
    ...
    //getClientesTable
    public function getCondicionesTable(Request $request){
        $lat='-12.0431800';
        $long='-77.0282400';
        $query = $request['query'];
        ...
        $url = $this->server2.'api/get_condiciones_tabla';
    ...
    public function deleteCondicion(Request $request){
        ...
        'lat'=>$lat,'long'=>$long,'idtipocondicion'=>$request['idtipocondicion']];
        $url = $this->server2.'api/eliminar_condicion';
        $result = $this->getServices($url,json_encode($data),session('usuario')->data[0]->token);
        return json_encode($result);
    }

    //addCondicion
    public function addCondicion(Request $request){
        $lat='-12.0431800';
        $long='-77.0282400';
        $data=['idusuario'=>session('usuario')->data[0]->info[0]->idusuario,'so'=>2,
        'lat'=>$lat,'long'=>$long,'condicion'=>$request['condicion']];
         $url = $this->server2.'api/agregar_condicion';
        $result = $this->getServices($url,json_encode($data),session('usuario')->data[0]->token);
        return json_encode($result);
    }
     ...
     public function editCondicion(Request $request){
        return view('gestionDatos/condiciones/editar',
        ['idtipocondicion'=>$request['idtipocondicion'],'condicion'=>$request['condicion']]);
    }
    ...
    public function updateCondicion(Request $request){
        ...
        $long='-77.0282400';
        $data=['idusuario'=>session('usuario')->data[0]->info[0]->idusuario,'so'=>2,
        'lat'=>$lat,'long'=>$long,'idtipocondicion'=>$request['idtipocondicion'],'condicion'=>$request['condicion']];
         $url = $this->server2.'api/editar_condicion';
        $result = $this->getServices($url,json_encode($data),session('usuario')->data[0]->token);
        return json_encode($result);
    }
    ...
    public function condiciones(Request $request){
        ...
        return view('gestionDatos/condiciones/inicio');
